...
const Geo = {
	...
	poly2svg : (polyline, height=400, width=400) => {
		...
		return `<svg xmlns="http://www.w3.org/2000/svg" height="${height}" width="${width}" preserveAspectRatio="xMinYMin meet" viewBox="${minX} ${minY} ${maxX - minX} ${maxY - minY}" stroke="currentColor" stroke-width="0.0002" fill="none"><g><path d="M${svgPath.join(' ')}"></path></g></svg>`
	},
	geoJSON: (polyline) => {
		let geo = {
